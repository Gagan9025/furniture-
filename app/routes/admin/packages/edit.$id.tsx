...
export async function loader({ params }: { params: { id: string } }) {
  ...
  const pkg = ContentManager.getPackageById(packageId);
...
export async function action({ request, params }: { request: Request; params: { id: string } }) {
  ...
  const updatedPackage = ContentManager.updatePackage(packageId, {
    name,
    description,
    image,
    materials,
...
export default function EditPackage() {
  const { pkg } = useLoaderData() as { pkg: any };
  
  return (
    <AdminLayout>
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-royal-silver-800">Edit Design Package</h1>
          ...
          <Form method="post" className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                name="materials"
                required
                defaultValue={pkg.materials.join(', ')}
